...
def generate_ipa(path, display_name):
    ...
    try:
        ...
        target_dir = os.path.join(TEMP_DIR, PAYLOAD_DIR)
        ipa_full_path = os.path.join(os.getcwd(), ipa_filename)

        with zipfile.ZipFile(ipa_full_path, 'w', zipfile.ZIP_DEFLATED) as ipa_zip:
            for root, dirs, files in os.walk(target_dir):
                for file in files:
                    file_path = os.path.join(root, file)
                    ipa_zip.write(
                        file_path,
                        os.path.relpath(file_path, os.path.join(target_dir, '..'))
                    )
